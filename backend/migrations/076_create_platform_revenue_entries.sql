...
INSERT INTO platform_revenue_entry_types (code, name) VALUES
...
CREATE TABLE platform_revenue_entries (
    ...
    UNIQUE KEY uq_platform_revenue_entries_assignment_type (
        assignment_id, entry_type_id
    ),
    KEY idx_platform_revenue_entries_job_created (
        job_id, created_at, id
    ),
    KEY idx_platform_revenue_entries_breakdown (
        financial_breakdown_id
    ),
    CONSTRAINT fk_platform_revenue_entries_assignment
        FOREIGN KEY (assignment_id) REFERENCES job_assignments(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_platform_revenue_entries_job
        FOREIGN KEY (job_id) REFERENCES jobs(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_platform_revenue_entries_type
        FOREIGN KEY (entry_type_id) REFERENCES platform_revenue_entry_types(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_platform_revenue_entries_breakdown
