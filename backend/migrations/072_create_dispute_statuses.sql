...
INSERT INTO dispute_statuses
    (code, name, is_terminal, sort_order)
VALUES
('OPEN', 'Open', 0, 10),
('UNDER_REVIEW', 'Under Review', 0, 20),
('RESOLVED', 'Resolved', 1, 30),
('REJECTED', 'Rejected', 1, 40),
('CANCELLED', 'Cancelled', 1, 50);
...
('QUALITY', 'Quality', 'Work quality is disputed.'),
('NO_SHOW', 'No Show', 'Provider or customer did not attend.'),
('DAMAGE', 'Damage', 'Property damage is disputed.'),
('PRICE', 'Price', 'Price or charges are disputed.'),
('SAFETY', 'Safety', 'A safety issue is reported.'),
('OTHER', 'Other', 'Another job issue is disputed.');
...
CREATE TABLE job_disputes (
    ...
    CONSTRAINT fk_job_disputes_assignment
        FOREIGN KEY (assignment_id)
        REFERENCES job_assignments (id)
        ON UPDATE RESTRICT
        ON DELETE CASCADE,
    CONSTRAINT fk_job_disputes_opener
        FOREIGN KEY (opened_by_user_id)
        REFERENCES users (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_job_disputes_type
        FOREIGN KEY (dispute_type_id)
        REFERENCES dispute_types (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_job_disputes_status
        FOREIGN KEY (status_id)
        REFERENCES dispute_statuses (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_job_disputes_resolver
