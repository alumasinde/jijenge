...
all 18 modules (Auth, Users, Providers, Services, Locations, Jobs,
Applications, Availability, Matching, Notifications, Payments, Financials,
Reviews, Verification, Trust, Execution, Disputes, plus system routes).
...
After these fixes: the app imports and boots cleanly (91 routes register),
`tools/security_audit.py` passes (no string-interpolated SQL across 188
files, no migration numbering conflicts across 98 migrations), and all 45
tests pass.
...
- `migrate.py --check` parses every pending migration (splitting on
