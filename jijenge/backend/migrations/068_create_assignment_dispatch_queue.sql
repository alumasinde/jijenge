...
(
    ...
    'A provider must confirm a new job assignment.'
),
(
    'ASSIGNMENT_CONFIRMED',
    'Assignment Confirmed',
    'A provider confirmed a job assignment.'
),
(
    'ASSIGNMENT_DECLINED',
    'Assignment Declined',
    'A provider declined a job assignment.'
);
...
CREATE TABLE assignment_dispatches (
    ...
    KEY idx_assignment_dispatches_status_expires (
        status_id, expires_at, id
    ),

    CONSTRAINT fk_assignment_dispatches_assignment
        FOREIGN KEY (assignment_id)
        REFERENCES job_assignments (id)
        ON UPDATE RESTRICT
        ON DELETE CASCADE,

    CONSTRAINT fk_assignment_dispatches_status
        FOREIGN KEY (status_id)
        REFERENCES assignment_dispatch_statuses (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
