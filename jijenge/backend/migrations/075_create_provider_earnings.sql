...
CREATE TABLE provider_earnings (
    ...
    processing_fee_amount DECIMAL(14,2) NOT NULL DEFAULT 0,
    ...
    UNIQUE KEY uq_provider_earnings_assignment (assignment_id),
    KEY idx_provider_earnings_provider_status (provider_id, status_id, created_at),
    KEY idx_provider_earnings_available (status_id, available_at, id),
    CONSTRAINT fk_provider_earnings_provider
        FOREIGN KEY (provider_id) REFERENCES provider_profiles(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_provider_earnings_assignment
        FOREIGN KEY (assignment_id) REFERENCES job_assignments(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_provider_earnings_breakdown
        FOREIGN KEY (financial_breakdown_id) REFERENCES job_financial_breakdowns(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_provider_earnings_status
        FOREIGN KEY (status_id) REFERENCES provider_earning_statuses(id)
        ON UPDATE RESTRICT ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
