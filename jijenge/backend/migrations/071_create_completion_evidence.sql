...
CREATE TABLE job_completion_evidence (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_job_completion_evidence_assignment_created (
        assignment_id, created_at, id
    ),
    CONSTRAINT fk_job_completion_evidence_assignment
        FOREIGN KEY (assignment_id)
        REFERENCES job_assignments (id)
        ON UPDATE RESTRICT
        ON DELETE CASCADE,
    CONSTRAINT fk_job_completion_evidence_type
        FOREIGN KEY (evidence_type_id)
        REFERENCES completion_evidence_types (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_job_completion_evidence_creator
        FOREIGN KEY (created_by_user_id)
        REFERENCES users (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT chk_job_completion_evidence_value
        CHECK (
